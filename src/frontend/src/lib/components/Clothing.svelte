<script lang="ts">
  ...
  let activeModalProduct: Product | null = null;
...
<div class="breadcrumb">ICPFA > MARKETPLACE > SHIRTS</div>
...
>
  {#each paginatedProducts as product, i}
    <div class="p-4 rounded-lg w-64 mx-auto">
      <img
        class="w-full h-48 object-cover mb-3 shadow-md"
        src={activeImages[i] === `front_${i}` ||
        activeImages[i] === "" ||
        product.shirtNumber === 0
          ? `/shirts/${product.teamId}.jpg`
          : `/shirts/${product.teamId}_${product.shirtNumber}.jpg`}
        alt="img"
      />
      <div class="flex gap-2 mb-3">
        <button
          class={`image-button focus:outline-none p-1 rounded transition-all duration-200 ${
            activeImages.includes(`front_${i}`)
              ? "border-2 border-gray-500"
              : "border-none"
          }`}
          on:click={() => (activeImages[i] = `front_${i}`)}
        >
          <img
            class="w-16 h-16 object-cover"
            src={`/shirts/${product.teamId}.jpg`}
            alt="Thumbnail 1"
          />
        </button>
        <button
          class={`image-button focus:outline-none p-1 rounded transition-all duration-200 ${
            activeImages.includes(`back_${i}`) && product.shirtNumber !== 0
              ? "border-2 border-gray-500"
              : "border-none"
          }`}
          on:click={() => (activeImages[i] = `back_${i}`)}
        >
          <img
            class="w-16 h-16 object-cover"
            src={product.shirtNumber === 0
              ? `/shirts/${product.teamId}.jpg`
              : `/shirts/${product.teamId}_${product.shirtNumber}.jpg`}
            alt="Thumbnail 2"
          />
      ...
      <div class="flex justify-between items-center">
        <div>
          ...
          <span class="text-gray-500 font-bold"
            >10.00 <span class="text-white">ICP</span></span
          >
        </div>
        <button
...
<div class="flex justify-center mt-4 items-center">
  <button
    class="bg-custom-blue hover:bg-custom-blue text-white font-bold m-2 py-2 px-2 rounded w-24 text-xs"
...
<ProductModal
  bind:product={activeModalProduct}
  showModal={!!activeModalProduct}
  on:close={() => (activeModalProduct = null)}
/>
